...
module.exports = class searchApi {
  ...
  findImdbId(){
    nameToImdb({ name: this.name }, (err, res, inf) => {
  ...
  getTorrents(torrent){
    ...
    for (let i = 0; i < torrent.length; i++){
      if (torrent[i].lang_id == 1 && torrent[i].seed > 15 &&
        ...
        result = [...result, {hash:hash, seed:torrent[i].seed, filename:title.trim().split(' ').join('.')}]
      }
    }
  ...
  getEpisodes(episodeRes){
    ...
    for (let i = 0; i < episode.length; i++){
      ...
      if (episode[i].torrent){
        ...
        if (torrent)
          result = [...result, {hash:torrent.hash, name:name, posterId:posterId, posterName:posterName, season:parseInt(season), episode:parseInt(ep), seed:torrent.seed,
          posterLink:'https://fileom.s3.amazonaws.com/serial/480/'+posterName, imdbCode:imdbId, id:id, title:serieTitle, filename:torrent.filename}]
      }
    }
    return (result)
  }
