...
module.exports = {
    ...
    loginForgotten: (req, res) => {
        const {
            errors,
            isValid
        } = validateLoginForgottenInput(req.body);
        if (!isValid) return res.status(400).json(errors);
    ...
    loginNewPassword: (req, res) => {
        const {
            errors,
            isValid
        } = validateLoginNewPasswordInput(req.body);
        if (!isValid) return res.status(400).json(errors);
    ...
    getSettings: (req, res) => {
        ...
        User.findOne({
            _id: userId
        }, ['email', 'firstName', 'lastName', 'lang', 'avatar', 'oAuth'], (err, data) => {
            if (err)
    ...
    getUserInfo: (req, res) => {
        ...
        User.findOne({
            username: username
        }, ['avatar', 'username', 'firstName', 'lastName'], (err, data) => {
            if (err)
                return res.status(500).send(err);

            Film.find({
                "is_seen.userId": data._id
            }, null, {
                limit: 10
            }, (err, films) => {
                if (err)
